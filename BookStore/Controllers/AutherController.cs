using BookStore.Models;
using BookStore.Repostories;
using Microsoft.AspNetCore.Http;
using Microsoft.AspNetCore.Mvc;

namespace BookStore.Controllers
{
    public class AutherController : Controller
    {
        public IBookStoreRepostory<Auther> AutherRepostory { get; }

        public AutherController(IBookStoreRepostory<Auther> autherRepostory )
        {
            AutherRepostory = autherRepostory;
        }
        // GET: AutherController
        public ActionResult Index()
        {
            var authers = AutherRepostory.GetAll();
            return View(authers);
        }

        // GET: AutherController/Details/5
        public ActionResult Details(int id)
        {
            var auther = AutherRepostory.Find(id);
            return View(auther);
        }

        // GET: AutherController/Create
        public ActionResult Create()
        {
            return View();
        }

        // POST: AutherController/Create
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Create(Auther  auther)
        {
            try
            {
                AutherRepostory.Add(auther);
                return RedirectToAction(nameof(Index));
            }
            catch
            {
                return View();
            }
        }

        // GET: AutherController/Edit/5
        public ActionResult Edit(int id)
        {
            var auther = AutherRepostory.Find(id);
            return View(auther);
        }

        // POST: AutherController/Edit/5
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(int id, Auther auther)
        {
            try
            {
                AutherRepostory.Update(id,auther);
                return Redirect("/Auther");
            }
            catch
            {
                return View();
            }
        }

        // GET: AutherController/Delete/5
        public ActionResult Delete(int id)
        {
            var auther = AutherRepostory.Find(id);
            return View(auther);
        }

        // POST: AutherController/Delete/5
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Delete(int id, Auther auther)
        {
            try
            {
                AutherRepostory.Delete(id);
                return RedirectToAction(nameof(Index));
            }
            catch
            {
                return View();
            }
        }
        public ActionResult Search(string trm)
        {

            var result = AutherRepostory.Search(trm);

            return View("Index", result);
        }
    }
}
